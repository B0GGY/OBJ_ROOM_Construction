...
class ISC_CH:
  ...
  def initialization(self):
    ...
    for cluster_id in range(self.k):
        representative_points = self.get_representatives(cluster_id, X, labels, L, D) # 这里0先做个测试，后面再换成各个簇的编号
  ...
  def input_signal(self, x):
    ...
    labels = self.adaptive_clustering()
    return labels
  ...
  def CH(self, label, dataset):
    # number of cluster can not be 1
    label = np.array(label)
    n_cluster = len(np.unique(label))
    cluster_dict = {}
    for i in range(n_cluster):
      cluster_dict[i] = dataset[label==i]
    c = np.mean(dataset, axis=0)
    numerator = 0
    denominator = 0
    for k in range(n_cluster):
      Gk = len(cluster_dict[k])
      ck = np.mean(cluster_dict[k], axis=0)
      numerator += Gk*(ck-c)@(ck-c).T
      for x in cluster_dict[k]:
        denominator += (x-ck)@(x-ck).T
    return (numerator/denominator)*((len(label)-n_cluster)/(n_cluster-1))

  def adaptive_clustering(self):
    max_CH = -np.inf
    best_k = 1
    for k in range(2, 8):
      self.k = k#self.calculate_eigen_gap(Z)
      labels, X, Z, eig_values, D, L, A = self.NJW(np.array(self.data_set))
      CH = self.CH(labels, np.array(self.data_set))
      # print(CH)
      if CH > max_CH:
        max_CH = CH
        best_k = k
