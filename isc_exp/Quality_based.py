...
class ISCQuatlity:
  ...
  def input_signal(self, x):
    ...
    labels, X, Z, eig_values, D, L, A = self.NJW(np.array(self.data_set))
    self.k = self.calculate_eigen_gap(Z)
    return labels
  ...
  def quality(self, eigen_mat):
    q = 0
    for i in range(eigen_mat.shape[0]):
      for j in range(eigen_mat.shape[1]):
        q += eigen_mat[i,j]**2/(max(eigen_mat[i,:])**2)-1
    return q

  def calculate_eigen_gap(self, eigvecs):
    max_quality = -np.inf
    best_k = 0
    for k in range(1, 15):
      q = self.quality(eigvecs[:,:k+1])
      # print('q {}'.format(q))
      if q > max_quality:
        max_quality = q
        best_k = k
    # print('---')
    # print(max_quality, best_k)
    return best_k
